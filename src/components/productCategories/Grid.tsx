...
export default function GridView({ categories, onEdit, onUpload }: GridProps) {
  return (
            ...
            <div className="p-5 flex-1 flex flex-col justify-between gap-4 bg-white">
              ...
              <div className="flex items-center justify-between gap-2 pt-4 border-t border-slate-50">
                <button
                  onClick={() => onEdit(cat)}
                  title="Edit Category"
                  className="flex items-center justify-center w-9 h-9 rounded-full bg-slate-50 text-slate-600 hover:text-[#1F3D32] hover:bg-[#1F3D32]/10"
                >
                  <svg
                    width="15"
                    height="15"
